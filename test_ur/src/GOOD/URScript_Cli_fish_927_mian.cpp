...
Description:
	...
	8. 把 z = 400mm 作为 零点
	9. 2018年 09月 27日 星期四 09:07:53 CST  把成员函数 写在里面 希望把二维码识别 重新做一遍
other:
...
public:
	...
	URScript()
	{
		...
		ar_sub = nh_.subscribe("/ar_pose_marker", 10, &URScript::AR_Callback,this);
	}
	~URScript()
	{
		ROS_INFO("~URScript");
	...
	void Execute_Selection_Mode()
	{
		...
		while(ros::ok()) {
			...
			srv.request.script = Selection_Mode (mode_x) ;	
			int count;
			while(!joint_state_client.call(srv) && ros::ok() ) //这个使得一直调用服务，直到成功call到服务.  
			{
				count++;
				cout<<"joint_state_client.call(srv)  "<< joint_state_client.call(srv)<<"count = " <<count <<endl;
			}
		}
	}
private:
