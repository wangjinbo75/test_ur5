...
Description:
	...
	8. 把 z = 400mm 作为 零点
	9. 2018年 09月 27日 星期四 09:07:53 CST  把成员函数 写在里面 希望把二维码识别 重新做一遍  获取当前的位置 ur里面设置机座 为参考系   加入多线程
	10. 添加示教功能  2018年 09月 28日 星期五 21:42:51 CST
other:
...
public:
	...
	{
		ROS_INFO("joint_pose = [ %.6f, %.6f, %.6f, %.6f, %.6f, %.6f ]",joint_pose[0],joint_pose[1],joint_pose[2],joint_pose[3],joint_pose[4],joint_pose[5]);
		ROS_INFO("tcp_pose =   [ %.6f, %.6f, %.6f, %.6f, %.6f, %.6f ]",tcp_pose[0],tcp_pose[1],tcp_pose[2],tcp_pose[3],tcp_pose[4],tcp_pose[5]);
	}
	string teach_mode()
	{
		stringstream temp;   
	...
	string Record_Coordinate_Location_And_Execute() 
	{
		cout << "Please move to a point : \n ";

		double joint_pose0[6] = {0};
		for(int i = 0; i < 6 ; i++) {
			joint_pose0[i] = joint_pose[i];
		}
		string  str = movej_joint( joint_pose0 , 1.4 , 1.05, 6.0, 0.0);
		return str;
	}
	string Selection_Mode(int mode_x)
	{
		...
		switch ( mode_x )  {
			...
			case 5 :{
					ROS_INFO(" mode 5 : Print_Coordinate_Info"); 
					Print_Coordinate_Info();
			        	break;
				}
			case 6 :{
					ROS_INFO(" mode 6 : teach_mode()");
					str = teach_mode();
					break;
				}
			case 7 :{
				//	ROS_INFO(" mode 6 : Record_Coordinate_Location_And_Execute");
					ROS_INFO(" mode 7 : end_teach_mode()");
					str = end_teach_mode();
					break;
				}
		return str;
	...
	void Execute_Selection_Mode()
	{
		...
		cout << "mode 6  :teach_mode  \n";
		cout << "mode 7  :end_teach_mode\n";
//		cout << "mode 7  :Record_Coordinate_Location_and_Execute\n";
		cout << "---------------------\n";
		...
		while (ros::ok())
		{
			int mode_x = Keyboard_Input();
			if (Selection_Mode(mode_x).size() != 0 ) 
			{
